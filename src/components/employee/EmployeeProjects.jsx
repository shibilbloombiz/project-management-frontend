...
export default function EmployeeProjects({
  ...
  const isMyTask = task => {
    return task.assigneeEmail && task.assigneeEmail.toLowerCase() === employeeEmail?.toLowerCase();
  };
  ...
  return <div className="text-left text-slate-100 font-sans space-y-6"> {} <div className="flex items-center justify-between flex-wrap gap-4"> <div className="flex items-center space-x-2.5"> <span className="p-2.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-2xl shadow-inner"> <FolderGit2 size={22} /> </span> <div> <h2 className="text-xl font-bold text-white tracking-tight font-sans">Project Workspace</h2> <p className="text-xs text-slate-400">Collaborative timeline workspace to check sprints, documents, and submit task logs.</p> </div> </div> </div> {projects.length === 0 ? <div className="bg-slate-800/40 border border-slate-700/50 rounded-3xl p-10 text-center space-y-4"> <span className="inline-flex p-4 bg-slate-700/20 border border-slate-755 rounded-full text-slate-400"> <HelpCircle size={30} /> </span> <div className="max-w-xs mx-auto space-y-1"> <h3 className="text-sm font-bold text-slate-200">No Projects Assigned</h3> <p className="text-[11px] text-slate-450 leading-relaxed font-semibold"> You aren't registered under any project staff lists. Please contact your company lead. </p> </div> </div> : <div className="space-y-6"> {} {selectedProject && <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4"> {[{
          ...
          bg: "bg-indigo-500/5"
              ...
              const myPending = p.tasks?.filter(t => t.assigneeEmail?.toLowerCase() === employeeEmail?.toLowerCase() && t.status !== "Done").length || 0;
              ...
              const filtered = selectedProject.tasks?.filter(task => {
                ...
                let matchesTab = true;
                if (taskTab === "my") matchesTab = isMine;else if (taskTab === "progress") matchesTab = isMine && task.status === "Dev";else if (taskTab === "review") matchesTab = isMine && task.status === "QA";else if (taskTab === "completed") matchesTab = isMine && task.status === "Done";else if (taskTab === "blocked") matchesTab = isMine && (task.note?.includes("[BLOCKED]") || task.status === "Planning");
