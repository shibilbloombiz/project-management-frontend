...
import { Briefcase, CreditCard, LogOut, MessageSquare, Trash2, Users, ChevronDown, ChevronRight, User } from 'lucide-react';
...
const itemClass = (isActive) =>
  `w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
    isActive
      ...
      : 'text-slate-550 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-850 hover:text-slate-850 dark:hover:text-slate-200'
  }`;
...
export default function CompanySidebar({ activeTab, onSelectTab, counts, org, onLogout, projectLeads = [], onSelectLead }) {
  const [leadsExpanded, setLeadsExpanded] = useState(false);

  return (
    <aside className="w-full md:w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-4 space-y-6 flex flex-col justify-between shrink-0 text-left">
      <div className="space-y-6">
        <div>
          <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-550 uppercase tracking-widest block mb-3 font-display px-2">
            Console Operations
          </span>
