    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-slate-100 text-sm font-semibold text-slate-600">
            ...
            <tr key={comp._id || comp.id} className="hover:bg-slate-50/50 transition-colors">
              <td className="py-4 px-6">
                <button 
                  ...
                  className="font-extrabold text-slate-800 hover:text-indigo-600 transition-colors block text-left font-display cursor-pointer"
                >
                  {comp.name}
                </button>
              ...
              <td className="py-4 px-6">
                <span className="px-2 py-0.5 text-[10px] font-bold rounded-full border bg-slate-50 text-indigo-700 border-indigo-100">
                  {comp.plan}
                </span>
              </td>
              <td className="py-4 px-6 text-slate-800 font-bold">{comp.users} Members</td>
              <td className="py-4 px-6 text-slate-800 font-bold">${comp.billing.toLocaleString()}</td>
              <td className="py-4 px-6">
          ...
          {companies.length === 0 && (
            <tr>
              <td colSpan="7" className="text-center py-8 text-slate-400 text-xs font-semibold">No companies match search filter query.</td>
            </tr>
          )}
        </tbody>
      </table>
